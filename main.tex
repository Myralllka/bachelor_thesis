...
The 21st century is a time of innovation and exploration in the applied science fields such as physics, medicine, biology and programming, as well as their intersections and fusions.
One of the most popular topics nowadays is an Autonomous Micro unmanned Aerial Vehicle (MAV).
MAVs became smaller, cheaper and more available for people.
The rising popularity and utility of MAVs is raising a common problem between them - reliance on the Global Positioning System(GPS), but due to GPS inaccuracy the MAV needs to have an obstacle avoidance system that is compact and reliable.

In this thesis the main focus is on developing a compact and reliable visual multi-camera obstacle avoidance system for micro aerial vehicles.
It covers such steps as stereo setup calibration and extracting the position of obstacles in the unknown environment from pairs of 2D images adhering to the real-time constraint.

Solution was designed in order to run onboard considering size and lifting limitations.
The performance of the prototype was measured in laboratory experiments.
The result proved that system is ready for on-drone deployment and real-life tests.
