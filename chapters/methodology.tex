...
This algorithm claims to be of the same accuracy as the state-of-the-art SIFT detector but a few times faster \cite{Sharif2017}. 
...
To minimize the distance between two lines, such $p_1$ and $p_2$ are needed that the line with directional vector $p_2 - p_1 = \vec{l}$ will be orthogonal to $d_1$ and $d_2$ which means
\begin{equation}
    \label{eq:ldd1}
